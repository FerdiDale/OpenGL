...
float getY (float x, float z) {
    float squaredSum = sqrt(pow(x, 2) + pow(z, 2));
    return ((2*sin(squaredSum))/squaredSum);
}
...
GLvoid drawScene(GLvoid) {
    ...
    for (float x = -10; x < 10; x+=cellSpace) {
        for (float z = -10; z < 10; z+=cellSpace) {
            currSquare[0] = (tVertex){x, getY(x,z), z};
            currSquare[1] = (tVertex){x, getY(x,z+cellSpace), z+cellSpace};
            currSquare[2] = (tVertex){x+cellSpace, getY(x+cellSpace,z+cellSpace), z+cellSpace};
            currSquare[3] = (tVertex){x+cellSpace, getY(x+cellSpace,z), z};
